...
if st.session_state.sched_df is not None:
    ...
    for _, row in sched_df.iterrows():
        ...
        if sh in grid and d in grid[sh]:
            grid[sh][d].append((row["Name"], str(row.get("Role", "")).lower(), bool(row["Fallback"])))
    ...
    # Legend BELOW the preview (Forced & MIT shown only if present) + "Current volunteer"
    legend = (
        ...
        with pd.ExcelWriter(buf, engine="xlsxwriter") as writer:
            ...
            for _, r in sched_df.iterrows():
                ...
                if sh in grid_x and d in grid_x[sh]:
                    grid_x[sh][d].append((r["Name"], str(r.get("Role", "")).lower(), bool(r["Fallback"])))
